 ...
 */
@SuppressWarnings("resource") @UmlClassDiagram(diagram = DiagramHdfsProxy.class)
...
public class HdfsProxyServer extends Server implements com.telenav.kivakit.filesystems.hdfs.proxy.spi.HdfsProxy
{
    ...
    private final SwitchParser<Folder> CONFIGURATION_FOLDER = folderSwitchParser(this,
            "configuration-folder", "Folder containing HDFS configuration files")
            .required()
            .build();

    private final SwitchParser<Integer> DATA_PORT = integerSwitchParser(this,
            "data-port", "The port to use when responding to data requests")
            .required()
            .build();

    private final SwitchParser<Integer> RMI_OBJECT_PORT = integerSwitchParser(this,
            "rmi-object-port", "The port to use when registering the remote proxy object")
            .required()
            .build();

    private final SwitchParser<UserGroupInformation> USERNAME = switchParser(UserGroupInformation.class)
            .name("username")
            ...
            Exception cause, String message, Object... arguments) throws RemoteException
    {
        ...
        throw new RemoteException(format(message, arguments), cause);
    }
}
